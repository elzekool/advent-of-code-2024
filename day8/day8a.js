...
antennaLines.forEach(([ groupX, groupY, groupLetter, groupDirX, groupDirY ]) => {
    ...
    // Keep track if we already found our start and end nodes as then the line stops
    ...
    while(true) {
        ...
        // Check that
        // - We not already placed this end of the line
        // - We are still in our map
        // - We are not trying to place it on the antenna itself
        //
        // If so, place antinode
        if (
            !placedStart &&
            inBound(curX, curY) && 
            antennas.filter(([x, y, letter]) => x === curX && y === curY && letter === groupLetter).length === 0
        ) {
            placedStart = true;
            if (antinodes.filter(([ aX, aY ]) => aX === curX && aY === curY).length === 0) {
        ...
        if (
            ...
            antennas.filter(([x, y, letter]) => x === curXInv && y === curYInv && letter === groupLetter).length === 0
        ) {
            placedEnd = true;
            if (antinodes.filter(([ aX, aY ]) => aX === curXInv && aY === curYInv).length === 0) {
